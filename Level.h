...
public:
	...
	int remain_enemies() const {
		int res = 0;
		for(const auto &i : times_of_enemies) res += i;
		return res;
	}
...
private:
	...
	std::vector<int> times_of_enemies;
	std::vector<std::tuple<int,double,double,double,double,int,int>>info_of_enemies;
};
